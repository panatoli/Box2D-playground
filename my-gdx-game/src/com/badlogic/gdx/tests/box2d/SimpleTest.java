...
public class SimpleTest extends Box2DTest {
	...
	protected void createWorld (World world) {
		...
		for (int i = 0; i < 2; i++) {
			...
			boxBodyDef.position.x = -24 + (float)((0.5) /*Math.random()*/ * 48 + i * 4);
			boxBodyDef.position.y = 10 + (float)(0.5f /*Math.random()*/ * 100);
		...
		for (int i = 0; i < 10; i++) {
			...
			circleBodyDef.position.x = -24 + (float)(0.5 /*Math.random()*/ * 48 + (i/4) * 4);
			circleBodyDef.position.y = 10 + (float)( 0.7 /*Math.random()*/ * 100 + (i % 4) * 10);
